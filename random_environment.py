...
class Environment:
    ...
    def _define_environment_space(self):
        ...
        # Whilst the latest free space block has not reached 0.2 from the right-hand edge of the environment,
        # continue adding free space blocks
    ...
    def draw_policy(self, policy):
        self.show(self.init_state)
        for i in range(len(policy)-1):
            current_state = policy[i]
            next_state = policy[i+1]
            cv2.line(self.image, (int(current_state[0] * self.magnification),
